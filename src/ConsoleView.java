...
Remember that Scanner connected to System.in, should be a class instance variable to prevent conflicts.
...
See Also:
Scanner.nextLine() Scanner System.in
     */
...
printHelp in interface GameView
    ...
    public void printHelp(){
        ...
        System.out.println("Game rules: You can have four active knights. As long as they are active, they won't heal, but they can gain XP by going on adventures. When you make a knight inactive, they will heal. How many monsters can you defeat before, you have to heal?");
    }
...
activeKnights - a list of Knight objects
    ...
    public void printBattleText(List<MOB> monsters, List<Knight> activeKnights){
        ...
        for(int i = 0; i < activeKnights.size(); i++){
            if(i < monsters.size()){
                System.out.printf("%-27s %-27s%n", activeKnights.get(i).getName(), monsters.get(i).getName());
            }else{
                System.out.printf("%-27s %-27s%n", activeKnights.get(i).getName(), "");
            }
        }
        System.out.println();
    }
...
Prints fortunes for the knight that they draw at the beginning of the quest by cycling through the provided list of knights. You do not have to worry about a special message if the fortune is null. Examples:
